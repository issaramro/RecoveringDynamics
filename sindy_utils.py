...
def sindy_library(X, poly_order, include_sine=False, include_names=False, exact_features=False):
    # Upgrade to combinations
    symbs = list('xyzuvwabcdefghijklmnopqrst')
...
def sindy_simulate(x0, t, Xi, poly_order, include_sine=False, exact_features=False):
    n = x0.size
    f = lambda x,t : np.dot(sindy_library(x.reshape((1,n)), poly_order, include_sine, include_names=False, exact_features=exact_features), Xi).reshape((n,))
    x = odeint(f, x0, t, mxstep=1000)
    return x
